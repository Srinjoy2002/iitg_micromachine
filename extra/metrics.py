...
def focus_stack(images):
    ...
    # Build the stacked image using the best-focus pixel from each layer.
...
def create_depth_map(focus_indices, layer_distance):
    ...
    # No additional smoothing: we want raw point cloud output.
    return depth_map
...
def depth_map_to_point_cloud(depth_map, focus_indices, num_images, xy_scale=1.0, z_scale=1.0):
    ...
    Colors are assigned based on Z height, using a 7-step ROYGBIV scale.
    """
    ...
    # Determine height range for color mapping based on Z
    if zs.size == 0:
        ...
        # Define 7 discrete ROYGBIV colors (in 0..1 RGB)
        roygbiv = np.array([
            [1.00, 0.00, 0.00],  # Red
        ...
        # Normalize each z value and map to a bin index [0..6]
        norm_z = (zs - z_min) / z_range
        bin_indices = (norm_z * 6).astype(int)
        bin_indices[bin_indices > 6] = 6  # Clamp maximum
        colors = roygbiv[bin_indices]
...
class PointCloudApp:
    ...
    def run_point_cloud_processing(self):
        # Reset the progress bar in the main thread.
        ...
        # Generate point cloud with ROYGBIV color coding based on height
        ...
        points_np = point_cloud  # already a numpy array (from our function)
