...
def enable():
    try:
        subprocess.run(["python", "E:\ACS motion Controller\python code\code\enable.py"], check=True)
...
def disable():
    try:
        subprocess.run(["python", "E:\ACS motion Controller\python code\code\disable.py"], check=True)
...
tk.Button(additional_frame, text="Focus Stack", width=15, command=focus_stack_handler).grid(row=0, column=1, padx=5, pady=2)
