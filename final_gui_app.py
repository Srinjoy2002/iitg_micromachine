...
IMAGE_FOLDER= f"dataset/captured_img_{timestamp}"
...
def run_focus_stack_script():
    ...
    """
    try:
        subprocess.run(["python", "focus_app.py"], check=True)
    except Exception as e:
...
tk.Label(title_frame, text="MOTION CONTROLLER", font=("Arial", 20, "bold")).pack(side=tk.LEFT, padx=10)
# Logo Image (Rightmost Corner)
logo_image = Image.open("download.png")  # Adjust the path
